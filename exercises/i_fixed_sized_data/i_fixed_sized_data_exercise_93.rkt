...
(define (happy-cham x)
  (big-bang (make-vcham x "default" 100)
    [on-tick toc]
    [to-draw render]
    [on-key modify-cham]
    [stop-when stop?]))
...
(check-expect (modify-cham (make-vcham 100 "white" 10) "w")
              (make-vcham 100 "white" 10))

(check-expect (modify-cham (make-vcham 100 "white" 10) "d")
              (make-vcham 100 "default" 10))
